...
class KeyManager private constructor(context: Context) {
    ...
    companion object {
        ...
        private const val KEYSTORE_ALIAS_PREFIX = "shieldmessenger_"
        private const val PREFS_NAME = "shield_messenger_keys"
     ...
     */
    fun rotatePinIfNeeded(cardManager: com.shieldmessenger.services.ContactCardManager, intervalMs: Long, maxDecrypts: Int): Boolean {
    ...
    fun getDatabasePassphrase(): ByteArray {
        ...
        try {
            ...
            val digest = MessageDigest.getInstance("SHA-256")
            digest.update(seed)
            digest.update("shield_messenger_database_v1".toByteArray(Charsets.UTF_8))
    ...
    fun getVoiceFileEncryptionKey(): ByteArray {
        ...
        try {
            ...
            val digest = MessageDigest.getInstance("SHA-256")
            digest.update(seed)
            digest.update("shield_messenger_voice_files_v1".toByteArray(Charsets.UTF_8))
    ...
    fun getImageFileEncryptionKey(): ByteArray {
        ...
        try {
            ...
            val digest = MessageDigest.getInstance("SHA-256")
            digest.update(seed)
            digest.update("shield_messenger_image_files_v1".toByteArray(Charsets.UTF_8))
