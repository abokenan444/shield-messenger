    ...
    private fun loadWalletInfo() {
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                val database = ShieldMessengerDatabase.getInstance(this@SendMoneyActivity, dbPassphrase)
        ...
        if (messageId.isNotEmpty()) {
            lifecycleScope.launch(Dispatchers.IO) {
                try {
                    val dbPassphrase = keyManager.getDatabasePassphrase()
                    val database = ShieldMessengerDatabase.getInstance(this@SendMoneyActivity, dbPassphrase)
                    database.messageDao().updatePaymentStatus(messageId, com.shieldmessenger.database.entities.Message.PAYMENT_STATUS_CANCELLED, "")
