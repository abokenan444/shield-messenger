...
import com.shieldmessenger.models.OutOfOrderBuffer
...
import com.shieldmessenger.IncomingCallActivity
...
class MessageService(private val context: Context) {
    ...
    companion object {
         ...
         *
         * @param database The ShieldMessenger database for persistence
         * @param message The message to retry
         */
        suspend fun scheduleRetry(database: ShieldMessengerDatabase, message: Message) {
            try {
    ...
    suspend fun sendVoiceMessage(
        ...
        try {
            ...
            val voiceRecorder = com.shieldmessenger.utils.VoiceRecorder(context)
    ...
    suspend fun sendPaymentRequest(
        ...
        try {
            ...
            val chatIntent = android.content.Intent("com.shieldmessenger.MESSAGE_RECEIVED")
        ...
        try {
            ...
            when {
                ...
                messageSequence > receiveCounter -> {
                    ...
                    val skippedKeys = mutableListOf<com.shieldmessenger.database.entities.SkippedMessageKey>()
                    ...
                    for (seq in receiveCounter until messageSequence) {
                        ...
                        skippedKeys.add(
                            com.shieldmessenger.database.entities.SkippedMessageKey(
                                id = "${contact.id}_$seq",
            ...
            when (callMessage) {
                is CallSignaling.CallSignalingMessage.CallOffer -> {
                    ...
                    val allowIncomingCallsWhenClosed = prefs.getBoolean(
                        com.shieldmessenger.SecurityModeActivity.PREF_ALLOW_INCOMING_CALLS_WHEN_CLOSED,
                    ...
                    if (!allowIncomingCallsWhenClosed) {
                        ...
                        val callHistory = com.shieldmessenger.database.entities.CallHistory(
                            ...
                            timestamp = System.currentTimeMillis(),
                            type = com.shieldmessenger.database.entities.CallType.MISSED,
                            duration = 0,
    ...
    suspend fun sendPingForMessage(message: Message): Result<String> = withContext(Dispatchers.IO) {
        try {
            ...
            TorService.getTransportGate()?.awaitOpen(com.shieldmessenger.network.TransportGate.TIMEOUT_SEND_MS)
    ...
    suspend fun pollForPongsAndSendMessages(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            ...
            TorService.getTransportGate()?.awaitOpen(com.shieldmessenger.network.TransportGate.TIMEOUT_QUICK_MS)
    ...
    suspend fun retryMessageNow(messageId: Long): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            database.messageDao().updateMessageStatus(messageId, com.shieldmessenger.database.entities.Message.STATUS_PING_SENT)
            ...
            if (hasPong) {
                ...
                if (retryResult.isSuccess) {
                    ...
                    database.messageDao().updateMessageStatus(messageId, com.shieldmessenger.database.entities.Message.STATUS_FAILED)
     ...
     */
    private suspend fun processBufferedMessages(contactId: Long, database: ShieldMessengerDatabase) {
    ...
    private suspend fun sendMessageBlobAfterPongAck(
        message: com.shieldmessenger.database.entities.Message,
        contact: com.shieldmessenger.database.entities.Contact,
        pingId: String
    ) {
        try {
            ...
            // Convert message type to wire protocol type byte
            val messageTypeByte: Byte = when (message.messageType) {
                com.shieldmessenger.database.entities.Message.MESSAGE_TYPE_VOICE -> 0x04.toByte() // VOICE
                com.shieldmessenger.database.entities.Message.MESSAGE_TYPE_IMAGE -> 0x09.toByte() // IMAGE
                com.shieldmessenger.database.entities.Message.MESSAGE_TYPE_PAYMENT_REQUEST -> 0x0A.toByte() // PAYMENT_REQUEST
                com.shieldmessenger.database.entities.Message.MESSAGE_TYPE_PAYMENT_SENT -> 0x0B.toByte() // PAYMENT_SENT
            ...
            if (success) {
                ...
                database.messageDao().updateMessageStatus(message.id, com.shieldmessenger.database.entities.Message.STATUS_SENT)
                ...
                database.messageDao().updateMessageStatus(message.id, com.shieldmessenger.database.entities.Message.STATUS_FAILED)
