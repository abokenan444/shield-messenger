...
class DownloadMessageService : Service() {
        ...
        serviceScope.launch {
            while (true) {
                ...
                try {
                    ...
                    val failureIntent = Intent("com.shieldmessenger.DOWNLOAD_FAILED")
    ...
    private class DownloadCtx(
        ...
        val pingId: String,
        val database: ShieldMessengerDatabase,
    ...
    private suspend fun downloadMessage(contactId: Long, contactName: String, pingId: String, connectionId: Long = -1L) {
        ...
        val database = ShieldMessengerDatabase.getInstance(this@DownloadMessageService, dbPassphrase)
    ...
    private suspend fun sendPongAndGetMessage(ctx: DownloadCtx): Boolean? {
        ...
        val alreadySentPong = withContext(Dispatchers.IO) {
            try {
                ...
                state == com.shieldmessenger.database.entities.PingInbox.STATE_PONG_SENT ||
                    state == com.shieldmessenger.database.entities.PingInbox.STATE_MSG_STORED
            } catch (e: Exception) {
    ...
    private suspend fun onDownloadSuccess(ctx: DownloadCtx) {
        ...
        val chatListIntent = Intent("com.shieldmessenger.NEW_PING")
        ...
        if (lastReceivedWireType == -1) {
            ...
            if (lastMsg != null) {
                lastReceivedWireType = when (lastMsg.messageType) {
                    com.shieldmessenger.database.entities.Message.MESSAGE_TYPE_TEXT -> 0x03
    ...
    private suspend fun onWatchdogTimeout(ctx: DownloadCtx) {
        ...
        val failIntent = Intent("com.shieldmessenger.DOWNLOAD_FAILED")
    ...
    private fun showSuccessNotification(contactName: String) {
        ...
        if (currentContactId != -1L &&
            com.shieldmessenger.ChatActivity.visibleContactId == currentContactId) {
    ...
    private suspend fun handleInstantMessageBlob(messageBytes: ByteArray, contactId: Long, contactName: String, pingId: String, connectionId: Long) {
        try {
            ...
            if (pingInbox == null) {
            ...
            } else if (pingInbox.state == com.shieldmessenger.database.entities.PingInbox.STATE_MSG_STORED) {
                ...
                if (pingInbox.state == com.shieldmessenger.database.entities.PingInbox.STATE_PING_SEEN) {
            ...
            when (typeByte.toInt()) {
                ...
                0x0F -> {
                    ...
                    val result = kotlin.runCatching {
                        database.withTransaction {
                            messageService.receiveMessage(
                                ...
                                senderPublicKey = senderPublicKey,
                                senderOnionAddress = contact.messagingOnion ?: "",
                                messageType = com.shieldmessenger.database.entities.Message.MESSAGE_TYPE_PROFILE_UPDATE,
                                pingId = pingId
                            )
                        }
                    }
                    ...
                    val refreshIntent = Intent("com.shieldmessenger.PROFILE_UPDATED")
                    ...
                    val msgReceivedIntent = Intent("com.shieldmessenger.MESSAGE_RECEIVED")
