...
use jni::sys::{
    jboolean, jbyte, jbyteArray, jint, jlong, jobjectArray, jstring, JNI_FALSE, JNI_TRUE,
};
use jni::JNIEnv;
...
use crate::audio::voice_streaming::{VoicePacket, VoiceStreamingListener};
use crate::crypto::{
    ...
    encrypt_message_with_evolution, evolve_chain_key, generate_keypair, sign_data,
    verify_signature,
};
...
pub extern "C" fn JNI_OnLoad(
    ...
    android_logger::init_once(
        android_logger::Config::default()
            ...
            .with_tag("ShieldMessenger-Rust"),
    );

    log::info!("ShieldMessenger native library loaded");
...
static GLOBAL_PING_RECEIVER: OnceCell<Arc<Mutex<mpsc::UnboundedReceiver<(u64, Vec<u8>)>>>> =
    OnceCell::new();
static GLOBAL_TAP_RECEIVER: OnceCell<Arc<Mutex<mpsc::UnboundedReceiver<Vec<u8>>>>> =
    OnceCell::new();
static GLOBAL_PONG_RECEIVER: OnceCell<Arc<Mutex<mpsc::UnboundedReceiver<(u64, Vec<u8>)>>>> =
    OnceCell::new();
static GLOBAL_ACK_RECEIVER: OnceCell<Arc<Mutex<mpsc::UnboundedReceiver<(u64, Vec<u8>)>>>> =
    OnceCell::new();
static GLOBAL_MESSAGE_RECEIVER: OnceCell<Arc<Mutex<mpsc::UnboundedReceiver<(u64, Vec<u8>)>>>> =
    OnceCell::new();
static GLOBAL_VOICE_RECEIVER: OnceCell<Arc<Mutex<mpsc::UnboundedReceiver<(u64, Vec<u8>)>>>> =
    OnceCell::new();
static GLOBAL_FRIEND_REQUEST_RECEIVER: OnceCell<Arc<Mutex<mpsc::UnboundedReceiver<Vec<u8>>>>> =
    OnceCell::new();

/// Global Voice Streaming Listener (v2.0)
static GLOBAL_VOICE_LISTENER: OnceCell<Arc<tokio::sync::Mutex<VoiceStreamingListener>>> =
    OnceCell::new();
...
static GLOBAL_RUNTIME: Lazy<tokio::runtime::Runtime> =
    Lazy::new(|| tokio::runtime::Runtime::new().expect("Failed to create global Tokio runtime"));
...
static SEND_SEMAPHORE: Lazy<tokio::sync::Semaphore> = Lazy::new(|| tokio::sync::Semaphore::new(6));

/// Global storage for Pong response channels
/// Maps ping_id -> oneshot sender for sending Pong bytes back to connection handler
static GLOBAL_PONG_SENDERS: OnceCell<
    Arc<Mutex<std::collections::HashMap<String, tokio::sync::oneshot::Sender<Vec<u8>>>>>,
> = OnceCell::new();
    ...
    catch_panic!(
        env,
        {
            ...
            let recipient_x25519_bytes =
                match jbytearray_to_vec(&mut env, recipient_x25519_public_key) {
                    Ok(v) => v,
                    Err(e) => {
                        let _ = env.throw_new("java/lang/IllegalArgumentException", e);
                        return std::ptr::null_mut();
            ...
            if recipient_x25519_bytes.len() != 32 {
                let _ = env.throw_new(
                    ...
                    "Invalid X25519 public key length (expected 32 bytes)",
                );
                return std::ptr::null_mut();
            }
            ...
            let key_manager = match crate::ffi::keystore::get_key_manager(&mut env, &context) {
                ...
                Err(e) => {
                    let _ = env.throw_new(
                        "java/lang/RuntimeException",
                        format!("Failed to get KeyManager: {}", e),
                    );
                    return std::ptr::null_mut();
                }
            };
            ...
            let our_x25519_private =
                match crate::ffi::keystore::get_encryption_private_key(&mut env, &key_manager) {
                    Ok(k) => k,
                    Err(e) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
                            format!("Failed to get encryption private key: {}", e),
                        );
                        return std::ptr::null_mut();
                    }
                };
            ...
            let our_x25519_public =
                match crate::ffi::keystore::get_encryption_public_key(&mut env, &key_manager) {
                    Ok(k) => k,
                    Err(e) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
                            format!("Failed to get encryption public key: {}", e),
                        );
                        return std::ptr::null_mut();
                    }
                };
            ...
            let mut shared_secret = match crate::crypto::key_exchange::derive_shared_secret(
                ...
                Err(e) => {
                    let _ =
                        env.throw_new("java/lang/RuntimeException", format!("ECDH failed: {}", e));
                    return std::ptr::null_mut();
                }
            };
    ...
    catch_panic!(
        env,
        {
            ...
            if wire_bytes.len() < 32 + 24 + 16 {
                let _ = env.throw_new(
                    "java/lang/IllegalArgumentException",
                    "Invalid wire message length (too short)",
                );
                return std::ptr::null_mut();
            }
                ...
                Ok(status) => {
                    ...
                    match string_to_jstring(&mut env, &status) {
                        ...
                        Err(_) => {
                            let _ = env.throw_new(
                                "java/lang/RuntimeException",
                                "Failed to create status string",
                            );
                            std::ptr::null_mut()
                        }
                    }
        ...
        env,
        {
            let cookie_path_str: String = match env.get_string(&cookie_path) {
                ...
                Err(_) => {
                    let _ = env.throw_new(
                        "java/lang/RuntimeException",
                        "Failed to read cookie path string",
                    );
                    return std::ptr::null_mut();
                }
            };
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_createHiddenService(
    ...
    catch_panic!(
        env,
        {
            ...
            let context = match env.call_static_method(
                ...
                Ok(val) => match val.l() {
                    ...
                    Err(_) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
                            "currentApplication returned null or invalid object",
                        );
                        return std::ptr::null_mut();
                    }
                },
            ...
            let hs_private_key = match crate::ffi::keystore::get_hidden_service_private_key(
                &mut env,
                &key_manager,
            ) {
                Ok(k) => k,
                Err(e) => {
                    let _ = env.throw_new(
                        "java/lang/RuntimeException",
                        format!("Failed to get hidden service key: {}", e),
                    );
                    return std::ptr::null_mut();
                }
            };
            ...
            let result = GLOBAL_RUNTIME.block_on(async {
                let mut manager = tor_manager.lock().unwrap();
                manager
                    .create_hidden_service(service_port as u16, local_port as u16, &hs_private_key)
                    .await
            });
            ...
            match result {
                Ok(onion_address) => match string_to_jstring(&mut env, &onion_address) {
                    ...
                    Err(_) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
                            "Failed to create onion address string",
                        );
                        std::ptr::null_mut()
                    }
                },
        ...
        env,
        {
            ...
            let domain_str: String = match env.get_string(&domain_sep) {
                ...
                Err(e) => {
                    ...
                    let _ = env.throw_new(
                        "java/lang/RuntimeException",
                        format!("Failed to read domain separator: {}", e),
                    );
                    return std::ptr::null_mut();
                }
            };
    ...
    catch_panic!(
        env,
        {
            match crate::storage::on_duress_pin_entered() {
                Ok(()) => {
                    log::info!(
                        "Duress PIN: core state cleared; app must wipe DB and optionally show fake"
                    );
                    JNI_TRUE
                }
                Err(e) => {
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_createVoiceHiddenService(
    ...
    catch_panic!(
        env,
        {
            ...
            let voice_private_key =
                match crate::ffi::keystore::get_voice_service_private_key(&mut env, &key_manager) {
                    Ok(k) => k,
                    Err(e) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
                            format!("Failed to get voice service key: {}", e),
                        );
                        return std::ptr::null_mut();
                    }
                };
            ...
            let result = GLOBAL_RUNTIME.block_on(async {
                let mut manager = tor_manager.lock().unwrap();
                manager
                    .create_voice_hidden_service(&voice_private_key)
                    .await
            });
            ...
            match result {
                Ok(onion_address) => match string_to_jstring(&mut env, &onion_address) {
                    ...
                    Err(_) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
                            "Failed to create voice onion address string",
                        );
                        std::ptr::null_mut()
                    }
                },
                ...
                match rx.try_recv() {
                    Ok((connection_id, ping_bytes)) => {
                        ...
                        if ping_bytes.is_empty() {
                            log::error!(
                                "FRAMING_VIOLATION: pollIncomingPing got empty buffer, conn_id={}",
                                connection_id
                            );
                            return std::ptr::null_mut();
                        }
                        ...
                        if ping_bytes[0] != MSG_TYPE_PING {
                            let head_hex: String = ping_bytes
                                .iter()
                                .take(8)
                                .map(|b| format!("{:02x}", b))
                                .collect::<Vec<_>>()
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_isConnectionAlive(
    ...
    catch_panic!(
        env,
        {
            ...
            if exists {
                log::debug!(
                    "Connection {} is alive (found in PENDING_CONNECTIONS)",
                    connection_id
                );
                1 // true
            } else {
                log::debug!(
                    "Connection {} is dead (not found in PENDING_CONNECTIONS)",
                    connection_id
                );
                0 // false
            }
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_sendPongBytes(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Sending encrypted Pong response ({} bytes) for connection {}",
                pong_bytes.len(),
                connection_id
            );
            ...
            let result = GLOBAL_RUNTIME.block_on(async {
                crate::network::TorManager::send_pong_response(connection_id as u64, &pong_bytes)
                    .await
            });
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_sendPongToNewConnection(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Sending Pong over NEW connection to {} ({} bytes)",
                onion_address,
                pong_bytes.len()
            );
            ...
            let result = GLOBAL_RUNTIME.block_on(async {
                ...
                let mut conn = {
                    ...
                    tor.connect(&onion_address, PING_PONG_HANDSHAKE_PORT)
                        .await?
                }; // Lock released
                ...
                log::info!(
                    "Pong sent successfully over new connection to {}",
                    onion_address
                );
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_sendPongToListener(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Sending Pong to listener at {}:{} ({} bytes)",
                onion_address,
                PING_PONG_HANDSHAKE_PORT,
                pong_bytes.len()
            );
            ...
            let result = GLOBAL_RUNTIME.block_on(async {
                ...
                log::info!(
                    "Pong sent successfully to listener at {}:{}",
                    onion_address,
                    PING_PONG_HANDSHAKE_PORT
                );
                Ok::<(), Box<dyn std::error::Error>>(())
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_sendPing(
    ...
    catch_panic!(
        ...
        {
            // Convert inputs
            let recipient_ed25519_bytes =
                match jbytearray_to_vec(&mut env, recipient_ed25519_pubkey) {
                    Ok(v) => v,
                    Err(e) => {
                        let _ = env.throw_new("java/lang/IllegalArgumentException", e);
                        return std::ptr::null_mut();
            ...
            let our_signing_private =
                match crate::ffi::keystore::get_signing_private_key(&mut env, &key_manager) {
                    Ok(k) => k,
                    Err(e) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
            ...
            if recipient_ed25519_bytes.len() != 32 {
                let _ = env.throw_new(
                    "java/lang/IllegalArgumentException",
                    format!(
                        "Invalid recipient Ed25519 public key length: expected 32 bytes, got {}",
                        recipient_ed25519_bytes.len()
                    ),
                );
                return std::ptr::null_mut();
            }

            let recipient_ed25519_verifying = match ed25519_dalek::VerifyingKey::from_bytes(
                &recipient_ed25519_bytes.as_slice().try_into().unwrap(),
            ) {
                Ok(pk) => pk,
                Err(e) => {
                    let _ = env.throw_new(
                        "java/lang/IllegalArgumentException",
                        format!("Invalid recipient Ed25519 pubkey: {}", e),
                    );
                    return std::ptr::null_mut();
                }
            };
            ...
            let our_x25519_public =
                match crate::ffi::keystore::get_encryption_public_key(&mut env, &key_manager) {
                    ...
                    Err(e) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
                            format!("Failed to get our X25519 pubkey: {}", e),
                        );
                        return std::ptr::null_mut();
                    }
                };
            ...
            let sender_x25519_pubkey: [u8; 32] = match our_x25519_public.as_slice().try_into() {
                ...
                Err(_) => {
                    let _ = env.throw_new(
                        "java/lang/IllegalArgumentException",
                        "Invalid sender X25519 pubkey size",
                    );
                    return std::ptr::null_mut();
                }
            };

            let recipient_x25519_pubkey: [u8; 32] =
                match recipient_x25519_bytes.as_slice().try_into() {
                    Ok(arr) => arr,
                    Err(_) => {
                        let _ = env.throw_new(
                            "java/lang/IllegalArgumentException",
                            "Invalid recipient X25519 pubkey size",
                        );
                        return std::ptr::null_mut();
                    }
                };
            ...
            if nonce_bytes.len() != 24 {
                let _ = env.throw_new(
                    "java/lang/IllegalArgumentException",
                    format!(
                        "Invalid ping_id length: expected 24 bytes, got {}",
                        nonce_bytes.len()
                    ),
                );
                return std::ptr::null_mut();
            ...
            log::info!(
                "Creating PingToken with provided ID: {} (timestamp: {})",
                &ping_id_str[..8.min(ping_id_str.len())],
                timestamp_secs
            );
            ...
            let ping_token = match crate::network::PingToken::with_nonce(
                ...
                Err(e) => {
                    let _ = env.throw_new(
                        "java/lang/RuntimeException",
                        format!("Failed to create Ping with nonce: {}", e),
                    );
                    return std::ptr::null_mut();
                }
            };
            ...
            let ping_bytes = match ping_token.to_bytes() {
                ...
                Err(e) => {
                    let _ = env.throw_new(
                        "java/lang/RuntimeException",
                        format!("Failed to serialize Ping: {}", e),
                    );
                    return std::ptr::null_mut();
                }
            };
            ...
            let encrypted_ping =
                match crate::crypto::encryption::encrypt_message(&ping_bytes, &shared_secret) {
                    Ok(enc) => enc,
                    Err(e) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
    ...
    catch_panic!(
        env,
        {
            ...
            if wire_message.len() != original_len {
                log::info!(
                    "LEGACY_WIRE_MIGRATED: prepended PING type byte ({} → {} bytes)",
                    original_len,
                    wire_message.len()
                );
            }
            ...
            log::info!(
                "Resending Ping with stored wire bytes ({} bytes) to {}",
                wire_message.len(),
                recipient_onion_str
            );
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_sendTap(
    ...
    catch_panic!(
        env,
        {
            ...
            let encrypted_tap = match crate::crypto::encryption::encrypt_message(
                tap_payload.as_bytes(),
                &shared_secret,
            ) {
                Ok(enc) => enc,
                Err(e) => {
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_sendFriendRequest(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Sending friend request to {} ({} bytes)",
                recipient_onion_str,
                friend_request_bytes.len()
            );
            ...
            for attempt in 1..=MAX_RETRIES {
                ...
                let result = runtime.block_on(async {
                    ...
                    let mut conn = manager
                        .connect(&recipient_onion_str, FRIEND_REQUEST_PORT)
                        .await?;
                    manager.send(&mut conn, &wire_message).await?;
                    Ok::<(), Box<dyn std::error::Error>>(())
                });
                ...
                match result {
                    Ok(_) => {
                        log::info!(
                            "Friend request sent successfully to {} on port {} (attempt {})",
                            recipient_onion_str,
                            FRIEND_REQUEST_PORT,
                            attempt
                        );
                    ...
                    Err(e) => {
                        log::warn!(
                            "Port {} attempt {}/{} failed: {}. Trying fallback port {}...",
                            FRIEND_REQUEST_PORT,
                            attempt,
                            MAX_RETRIES,
                            e,
                        ...
                        let fallback_result = runtime.block_on(async {
                            let manager = tor_manager.lock().unwrap();
                            let mut conn =
                                manager.connect(&recipient_onion_str, FALLBACK_PORT).await?;
                            manager.send(&mut conn, &wire_message).await?;
                            Ok::<(), Box<dyn std::error::Error>>(())
                        });
                        ...
                        match fallback_result {
                            Ok(_) => {
                                log::info!(
                                    "Friend request sent via fallback port {} (attempt {})",
                                    FALLBACK_PORT,
                                    attempt
                                );
                                return 1;
                            ...
                            Err(fallback_err) => {
                                if attempt < MAX_RETRIES {
                                    ...
                                    log::error!(
                                        "Failed to send friend request to {} after {} attempts: {}",
                                        recipient_onion_str,
                                        MAX_RETRIES,
                                        fallback_err
                                    );
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_sendFriendRequestAccepted(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Sending friend request accepted to {} ({} bytes)",
                recipient_onion_str,
            ...
            for attempt in 1..=MAX_RETRIES {
                ...
                match result {
                    Ok(_) => {
                        log::info!(
                            "Friend request acceptance sent to {} on port {} (attempt {})",
                            recipient_onion_str,
                            FRIEND_REQUEST_PORT,
                            attempt
                        );
        ...
        env,
        {
            ...
            match TorManager::new() {
                ...
                Err(e) => {
                    ...
                    let _ = env.throw_new(
                        "java/lang/RuntimeException",
                        format!("TorManager reset failed: {}", e),
                    );
                }
            }
        },
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_sendAckOnConnection(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Sending {} on connection {}: {} bytes (item_id={})",
                ack_type_str,
                connection_id,
                ack_bytes.len(),
                item_id_str
            ...
            let result = runtime.block_on(async {
                crate::network::TorManager::send_ack_on_connection(
            ...
            match result {
                Ok(_) => {
                    log::info!(
                        "{} sent successfully on connection {} (item_id={})",
                        ack_type_str,
                        connection_id,
                        item_id_str
                    );
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_startTapListener(
    ...
    catch_panic!(
        ...
        {
            log::info!(
                "Starting multiplexed listener on port {} (TAP + FRIEND_REQUEST)",
                port
            );
            ...
            match result {
                Ok((mut ping_receiver, _pong_receiver)) => {
                    ...
                    log::info!(
                        "Friend request channel initialized successfully (shared port {})",
                        port
                    );
                    ...
                    GLOBAL_RUNTIME.spawn(async move {
                        while let Some((_connection_id, tap_bytes)) = ping_receiver.recv().await {
                            log::info!(
                                "Received message via multiplexed listener: {} bytes",
                                tap_bytes.len()
                            );
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_decryptIncomingTap(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Decrypting tap from sender X25519: {}",
            ...
            // Decrypt tap
            let decrypted =
                match crate::crypto::encryption::decrypt_message(encrypted_tap, &shared_secret) {
                    Ok(plain) => plain,
                    Err(e) => {
                ...
                match rx.try_recv() {
                    Ok((conn_id, pong_bytes)) => {
                        ...
                        if pong_bytes.is_empty() {
                            log::error!(
                                "FRAMING_VIOLATION: pollIncomingPong got empty buffer, conn_id={}",
                                conn_id
                            );
                            return std::ptr::null_mut();
                        }
                        ...
                        if pong_bytes[0] != MSG_TYPE_PONG {
                            let head_hex: String = pong_bytes
                                .iter()
                                .take(8)
                                .map(|b| format!("{:02x}", b))
                                .collect::<Vec<_>>()
                        ...
                        log::info!(
                            "POLL: got pong len={} conn={} head={:02x}{:02x}{:02x}{:02x}",
                            pong_bytes.len(),
                            conn_id,
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_decryptAndStorePongFromListener(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Decrypting pong from recipient X25519: {}",
                hex::encode(&recipient_x25519_pubkey)
            );
            ...
            let decrypted_pong =
                match crate::crypto::encryption::decrypt_message(encrypted_pong, &shared_secret) {
            ...
            let pong_token: crate::protocol::message::PongToken =
                match bincode::deserialize(&decrypted_pong) {
                    Ok(token) => token,
                    Err(e) => {
            ...
            if pong_token.protocol_version != crate::network::tor::P2P_PROTOCOL_VERSION {
                log::error!(
                    "PROTOCOL_MISMATCH: Listener Pong protocol_version={} ours={} — dropping",
                    pong_token.protocol_version,
            ...
            let expected_signer = expected_signer.or_else(|| {
                log::info!(
                    "In-memory signer miss for ping_id={}, trying DB fallback...",
                    ping_id
                );
                let db_result = lookup_signer_db(&mut env, &ping_id);
                if db_result.is_some() {
                    log::info!(
                        "DB_SIGNER_HIT: Found signer in Room DB for ping_id={}",
                        ping_id
                    );
                } else {
                    log::warn!(
                        "DB_SIGNER_MISS: No signer in Room DB either for ping_id={}",
                        ping_id
                    );
                }
                db_result
                            ...
                            match pingpong_token.verify(&verifying_key) {
                                Ok(true) => {
                                    ...
                                    VERIFIED_PONG_IDS
                                        .lock()
                                        .unwrap()
                                        .insert(ping_id.clone(), std::time::Instant::now());
            ...
            log::info!(
                "Stored verified Pong in GLOBAL_PONG_SESSIONS (count={})",
    ...
    catch_panic!(
        env,
        {
            ...
            loop {
                ...
                if let Some(_pong_session) =
                    crate::network::pingpong::get_pong_session(&ping_id_str)
                {
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_sendMessageBlob(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Sending message blob to {} ({} bytes encrypted message)",
                onion_address,
                message_bytes.len()
            );
            ...
            if !is_crdt_type && message_bytes.len() < 49 {
                log::error!(
                    "Encrypted payload too short: {} bytes (minimum 49)",
                    message_bytes.len()
                );
                return 0;
            }
            ...
            log::info!(
                "Wire message: {} bytes (1-byte type + 32-byte X25519 pubkey + {} bytes encrypted)",
                wire_message.len(),
                message_bytes.len()
            );
            ...
            let result = GLOBAL_RUNTIME.block_on(async {
                ...
                let _permit = SEND_SEMAPHORE.acquire().await.map_err(|_| {
                    Box::new(std::io::Error::new(
                        std::io::ErrorKind::Other,
                        "Send semaphore closed",
                ...
                tokio::time::timeout(timeout_duration, async {
                    ...
                    let mut conn = crate::network::tor::connect_to_onion(&onion_address, port)
                        .await
                        .map_err(|e| {
                            Box::new(std::io::Error::new(
            ...
            match result {
                ...
                Err(e) => {
                    ...
                    if error_msg.contains("timed out") || error_msg.contains("timeout") {
                        ...
                        log::error!(
                            "BLOB_SEND_FAIL kind=SOCKS_TIMEOUT to {}: {}",
                            onion_address,
                            e
                        );
                        ...
                        || error_msg.contains("connect")
                    {
                        ...
                        log::error!(
                            "BLOB_SEND_FAIL kind=CONNECT_ERR to {}: {}",
                            onion_address,
                            e
                        );
                    } else if error_msg.contains("tor")
                        || error_msg.contains("bootstrap")
                        || error_msg.contains("circuit")
                    {
                        BLOB_FAIL_TOR_NOT_READY.fetch_add(1, Ordering::Relaxed);
                        log::error!(
                            "BLOB_SEND_FAIL kind=TOR_NOT_READY to {}: {}",
                            onion_address,
                            e
                        );
                        ...
                        || error_msg.contains("broken pipe")
                    {
                        ...
                        log::error!(
                            "BLOB_SEND_FAIL kind=UNKNOWN_ERR to {}: {}",
                            onion_address,
                            e
                        );
                    }
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_sendHttpToVoiceOnion(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Sending call signaling via HTTP POST to voice onion {} ({} bytes encrypted)",
                onion_address,
                message_bytes.len()
            );
            ...
            let result = GLOBAL_RUNTIME.block_on(async {
                ...
                let mut stream = match crate::audio::voice_streaming::connect_via_socks5(
                    &onion_address,
                    9152,
                    0,
                    "signaling",
...
fn cleanup_verified_pong_ids(env: &mut JNIEnv) {
    ...
    // Clean up old VERIFIED_PONG_IDS entries
    {
        ...
        if removed > 0 {
            log::info!(
                "VERIFIED_PONG_IDS cleanup: removed {} expired entries ({} remaining)",
                removed,
                verified.len()
            );
        }
    ...
    if !verified_ids.is_empty() {
        ...
        for id in &verified_ids {
            if signers.remove(id).is_some() {
                log::debug!(
                    "Deferred signer cleanup: removed OUTGOING_PING_SIGNERS entry for ping_id={}",
                    &id[..8.min(id.len())]
                );
...
fn store_pending_ping_db(env: &mut JNIEnv, ping_id: &str, signer_pubkey: &[u8; 32]) {
    let class = match env.find_class("com/shieldmessenger/database/PendingPingStore") {
        ...
        Err(_) => {
            log::warn!(
                "store_pending_ping_db: PendingPingStore class not found (DAO not initialized?)"
            );
            return;
        }
    };
    let j_ping_id = match env.new_string(ping_id) {
        Ok(s) => s,
        Err(e) => {
            log::warn!("store_pending_ping_db: new_string failed: {}", e);
            return;
        }
    };
    let j_signer = match env.byte_array_from_slice(signer_pubkey) {
        Ok(a) => a,
        Err(e) => {
            log::warn!("store_pending_ping_db: byte_array failed: {}", e);
            return;
        }
    };
...
fn lookup_signer_db(env: &mut JNIEnv, ping_id: &str) -> Option<[u8; 32]> {
    ...
    let j_ping_id = match env.new_string(ping_id) {
        Ok(s) => s,
        Err(e) => {
            log::warn!("lookup_signer_db: new_string failed: {}", e);
            return None;
        }
    };
    ...
    match jbytearray_to_vec(env, j_bytes) {
        ...
        Ok(bytes) => {
            log::warn!(
                "lookup_signer_db: unexpected signer length {} (expected 32)",
                bytes.len()
            );
            None
        }
    ...
    catch_panic!(
        env,
        {
            ...
            if wire_bytes.len() < MIN_LEN {
                ...
                let _ = env.throw_new(
                    "java/lang/IllegalArgumentException",
                    "FRAMING_PROTOCOL_VIOLATION: Wire message too short",
                );
                return std::ptr::null_mut();
            }
            ...
            log::info!(
                "Decrypting incoming Ping: offset={}, encrypted_len={} bytes",
                OFFSET,
                encrypted_ping.len()
            );
            ...
            let our_x25519_private =
                match crate::ffi::keystore::get_encryption_private_key(&mut env, &key_manager) {
                    ...
                    Err(e) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
                            format!("Failed to get X25519 private key: {}", e),
                        );
                        return std::ptr::null_mut();
                    }
                };
            ...
            let decrypted_ping =
                match crate::crypto::encryption::decrypt_message(encrypted_ping, &shared_secret) {
                    Ok(plaintext) => plaintext,
                    Err(e) => {
                        let _ = env.throw_new(
                            "java/lang/SecurityException",
                            format!("Ping decryption failed: {}", e),
                        );
                        return std::ptr::null_mut();
                    }
                };
            ...
            log::info!(
                "Ping decrypted successfully: {} bytes",
                decrypted_ping.len()
            );
            ...
            let ping_token = match crate::network::PingToken::from_bytes(&decrypted_ping) {
                ...
                Err(e) => {
                    ...
                    log::error!("Reason: {}", e);
                    log::error!(
                        "Decrypted {} bytes - wrong payload type for PING",
                        decrypted_ping.len()
                    );
                    let _ = env.throw_new(
                        "java/lang/RuntimeException",
                        format!("PAYLOAD_PARSE_FAILURE: Failed to parse PingToken: {}", e),
                    );
                    return std::ptr::null_mut();
                }
            };
            ...
            if ping_token.protocol_version != crate::network::tor::P2P_PROTOCOL_VERSION {
                log::error!(
                    "PROTOCOL_MISMATCH: Ping protocol_version={} ours={} — rejecting",
                    ping_token.protocol_version,
                    crate::network::tor::P2P_PROTOCOL_VERSION
                );
                let _ = env.throw_new(
                    "java/lang/SecurityException",
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_respondToPing(
    ...
    catch_panic!(
        env,
        {
            ...
            let ping_token = {
                ...
                match stored.remove(&ping_id_str) {
                    ...
                    None => {
                        let _ = env.throw_new(
                            "java/lang/IllegalArgumentException",
                            format!("Ping ID not found: {}", ping_id_str),
                        );
                        return std::ptr::null_mut();
                    }
                }
            ...
            let recipient_keypair = match ed25519_dalek::SigningKey::from_bytes(
                &our_signing_private.as_slice().try_into().unwrap(),
            ) {
            ...
            // Create PongToken
            let pong_token =
                match crate::network::PongToken::new(&ping_token, &recipient_keypair, true) {
                    Ok(token) => token,
                    Err(e) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
                            format!("Failed to create Pong: {}", e),
                        );
                        return std::ptr::null_mut();
                    }
                };
            ...
            let pong_bytes = match pong_token.to_bytes() {
                ...
                Err(e) => {
                    let _ = env.throw_new(
                        "java/lang/RuntimeException",
                        format!("Failed to serialize Pong: {}", e),
                    );
                    return std::ptr::null_mut();
                }
            };
            ...
            log::info!(
                "Pong created successfully for Ping {}",
                hex::encode(&ping_token.nonce)
            );
            ...
            // Encrypt Pong token
            let encrypted_pong =
                match crate::crypto::encryption::encrypt_message(&pong_bytes, &shared_secret) {
                    Ok(ciphertext) => ciphertext,
                    Err(e) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
                            format!("Pong encryption failed: {}", e),
                        );
                        return std::ptr::null_mut();
                    }
                };
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_decryptIncomingPong(
    ...
    catch_panic!(
        env,
        {
            ...
            if wire_bytes.len() < MIN_LEN {
                log::warn!(
                    "Pong wire too short: actual_len={}, min_required={}",
                    wire_bytes.len(),
                    MIN_LEN
                );
                let _ = env.throw_new(
                    "java/lang/IllegalArgumentException",
                    "Wire message too short - missing X25519 pubkey",
                );
                return std::ptr::null_mut();
            }
            ...
            log::info!(
                "Attempting to decrypt incoming Pong: offset={}, encrypted_len={} bytes",
                OFFSET,
                encrypted_pong.len()
            );
            ...
            if pong_token.protocol_version != crate::network::tor::P2P_PROTOCOL_VERSION {
                log::error!(
                    "PROTOCOL_MISMATCH: Pong protocol_version={} ours={} — rejecting",
                    pong_token.protocol_version,
                    crate::network::tor::P2P_PROTOCOL_VERSION
                );
                let _ = env.throw_new(
            ...
            log::info!(
                "Decrypted incoming Pong for Ping ID: {} (authenticated={})",
                ping_id,
                pong_token.authenticated
            );
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_sendDirectMessage(
    ...
    catch_panic!(
        env,
        {
            ...
            let sender_keypair = ed25519_dalek::SigningKey::from_bytes(
                &our_signing_private.as_slice().try_into().unwrap(),
            );
            ...
            let ping_token = match crate::network::PingToken::new(
                ...
                Err(e) => {
                    let _ = env.throw_new(
                        "java/lang/RuntimeException",
                        format!("Failed to create Ping: {}", e),
                    );
                    return 0;
                }
            };
            ...
            let encrypted_ping =
                match crate::crypto::encryption::encrypt_message(&ping_bytes, &shared_secret) {
                    ...
                    Err(e) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
                            format!("Ping encryption failed: {}", e),
                        );
                        return 0;
                    }
                };
    ...
    catch_panic!(
        env,
        {
            ...
            let result = runtime.block_on(async {
                ...
                log::info!(
                    "Successfully read encrypted message ({} bytes)",
                    message_bytes.len()
                );
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_startBootstrapListener(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Starting bootstrap event listener (ControlSocket: {})...",
...
fn invoke_tor_event_callback(
    event: crate::network::tor::TorEventType,
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_createPingToken(
    ...
    catch_panic!(
        env,
        {
            ...
            let our_signing_public =
                match crate::ffi::keystore::get_signing_public_key(&mut env, &key_manager) {
                    Ok(k) => k,
                    Err(e) => {
                        let _ = env.throw_new(
                            "java/lang/RuntimeException",
                            format!("Failed to get public key: {}", e),
                        );
                        return std::ptr::null_mut();
                    }
                };
            ...
            let sender_keypair = match ed25519_dalek::SigningKey::from_bytes(
                &our_signing_private.as_slice().try_into().unwrap(),
            ) {
                signing_key => ed25519_dalek::SigningKey::from(signing_key),
            };

            let recipient_ed25519_pubkey = match ed25519_dalek::VerifyingKey::from_bytes(
                &recipient_ed25519_bytes.as_slice().try_into().unwrap(),
            ) {
                Ok(pk) => pk,
                Err(e) => {
                    let _ = env.throw_new(
                        "java/lang/IllegalArgumentException",
                        format!("Invalid recipient pubkey: {}", e),
                    );
                    return std::ptr::null_mut();
                }
            };
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_nativeDecryptPingToken(
    ...
    catch_panic!(
        env,
        {
            ...
            // 5. Decrypt
            let ping_bytes = match crate::crypto::encryption::decrypt_message(
                &encrypted_bytes,
                &shared_secret,
            ) {
                Ok(bytes) => bytes,
    ...
    catch_panic!(
        env,
        {
            ...
            let stored_session = match crate::network::get_ping_session(&ping_id_str) {
                ...
                None => {
                    let _ =
                        env.throw_new("java/lang/IllegalStateException", "Ping session not found");
                    return std::ptr::null_mut();
                }
            };
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_nativeDecryptPongToken(
    ...
    catch_panic!(
        env,
        {
            ...
            // 5. Decrypt
            let pong_bytes = match crate::crypto::encryption::decrypt_message(
                &encrypted_bytes,
                &shared_secret,
            ) {
                Ok(bytes) => bytes,
            ...
            let authenticated = if pong_token.authenticated {
                "true"
            } else {
                "false"
            };
    ...
    catch_panic!(
        env,
        {
            ...
            match crate::crypto::key_exchange::derive_public_key(&private_key_bytes) {
                Ok(public_key) => match vec_to_jbytearray(&mut env, &public_key) {
                    Ok(arr) => arr.into_raw(),
                    Err(e) => {
                        let _ = env.throw_new("java/lang/RuntimeException", e);
                        std::ptr::null_mut()
    ...
    catch_panic!(
        env,
        {
            ...
            match crate::crypto::key_exchange::derive_shared_secret(&our_private, &their_public) {
                Ok(shared_secret) => match vec_to_jbytearray(&mut env, &shared_secret) {
                    Ok(arr) => arr.into_raw(),
                    Err(e) => {
                        let _ = env.throw_new("java/lang/RuntimeException", e);
                        std::ptr::null_mut()
    ...
    catch_panic!(
        env,
        {
            ...
            if shared_secret_vec.len() != 32 && shared_secret_vec.len() != 64 {
                let _ = env.throw_new(
                    "java/lang/IllegalArgumentException",
                    "Shared secret must be 32 or 64 bytes",
                );
                return std::ptr::null_mut();
            }
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_encryptMessageWithEvolutionJNI(
    ...
    catch_panic!(
        env,
        {
            ...
            match encrypt_message_with_evolution(
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_decryptMessageWithEvolutionJNI(
    ...
    catch_panic!(
        env,
        {
            ...
            match decrypt_message_with_evolution(
                &encrypted_vec,
                &mut chain_key_array,
    ...
    catch_panic!(
        env,
        {
            ...
            let nonce: [u8; 24] = match ciphertext_vec[9..33].try_into() {
                ...
                Err(_) => {
                    let _ =
                        env.throw_new("java/lang/IllegalArgumentException", "Invalid nonce length");
                    return std::ptr::null_mut();
                }
            };
            ...
            match cipher.decrypt(nonce_obj, encrypted_payload) {
                Ok(plaintext_bytes) => match String::from_utf8(plaintext_bytes) {
                    Ok(plaintext_str) => match env.new_string(&plaintext_str) {
                        Ok(jstr) => jstr.into_raw(),
                        Err(e) => {
                            let _ = env.throw_new(
                                "java/lang/RuntimeException",
...
use crate::crypto::ack_state::{reset_ack_state, validate_and_record_ack};
use crate::crypto::encryption::{
    commit_ratchet_advancement, encrypt_message_deferred, rollback_ratchet_advancement,
    store_pending_ratchet_advancement,
};
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_encryptMessageDeferred(
    ...
    catch_panic!(
        env,
        {
            ...
            match encrypt_message_deferred(
                plaintext_str.as_bytes(),
                &chain_key_array,
                sequence as u64,
            ) {
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_storePendingRatchetAdvancement(
    ...
    catch_panic!(
        env,
        {
            ...
            match store_pending_ratchet_advancement(
                &contact_id_str,
                &message_id_str,
                chain_key_array,
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_sendDeliveryAck(
    ...
    catch_panic!(
        env,
        {
            ...
            let encrypted_ack =
                match crate::crypto::encryption::encrypt_message(&ack_bytes, &shared_secret) {
                    Ok(enc) => enc,
                    Err(e) => {
        ...
        env,
        {
            ...
            match result {
                Ok((mut ping_receiver, _pong_receiver)) => {
                    ...
                    let _ = crate::network::tor::ACK_TX
                        .set(Arc::new(std::sync::Mutex::new(tx.clone())));
                    ...
                    GLOBAL_RUNTIME.spawn(async move {
                        while let Some((connection_id, ack_bytes)) = ping_receiver.recv().await {
                            log::info!(
                                "Received ACK via port 9153 listener: {} bytes",
                                ack_bytes.len()
                            );
                    ...
                    log::info!(
                        "ACK listener started successfully on port {} with shared channel",
                        port
                    );
                    1 // success
                }
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_decryptAndStoreAckFromListener(
    ...
    catch_panic!(
        env,
        {
            ...
            if wire_bytes.len() < MIN_LEN {
                log::error!(
                    "ACK wire too short: actual_len={}, min_required={}",
                    wire_bytes.len(),
                    MIN_LEN
                );
                return std::ptr::null_mut();
            ...
            // Extract sender's X25519 public key (after type byte at offset 1)
            let sender_x25519_pubkey: [u8; 32] =
                wire_bytes[OFFSET..OFFSET + 32].try_into().unwrap();
            let encrypted_ack = &wire_bytes[OFFSET + 32..];

            log::info!(
                "Decrypting ACK: offset={}, sender_x25519={}, encrypted_len={}",
                OFFSET,
                hex::encode(&sender_x25519_pubkey),
                encrypted_ack.len()
            );
            ...
            // Decrypt ACK
            let ack_bytes =
                match crate::crypto::encryption::decrypt_message(encrypted_ack, &shared_secret) {
            ...
            log::info!(
                "RECEIVED DELIVERY ACK: item_id={}, ack_type={}",
                item_id,
                ack_type
            );
            ...
            let (signer_bytes, used_fallback) = match stored_signer {
                Some(stored) => {
                    log::info!(
                        "ACK_VERIFY: Using stored signer (fast path) for {} item_id={}",
                        ack_type,
                        item_id
                    );
                    (stored, false)
            ...
            match ed25519_dalek::VerifyingKey::from_bytes(&signer_bytes) {
                Ok(verifying_key) => match ack_token.verify(&verifying_key) {
                    Ok(true) => {
                        log::info!(
                            "ACK_SIG_OK: {} signature verified for item_id={} (fallback={})",
                            ack_type,
                            item_id,
                            used_fallback
                        );
                    ...
                    Err(e) => {
                        log::error!(
                            "ACK_SIG_ERROR: {} sig check error for item_id={}: {} — dropping",
                            ack_type,
                            item_id,
                            e
                        );
                ...
                Err(e) => {
                    log::error!(
                        "ACK_SIG_ERROR: Invalid signer pubkey for item_id={}: {} — dropping",
                        item_id,
                        e
                    );
                    return std::ptr::null_mut();
            ...
            let ack_json = if used_fallback {
                ...
                format!(
                    r#"{{"item_id":"{}","ack_type":"{}","fallback":false}}"#,
                    item_id, ack_type
                )
            };
        ...
        env,
        {
            ...
            let json = match quote.to_json() {
                ...
                Err(e) => {
                    let _ = env.throw_new(
                        "java/lang/RuntimeException",
                        format!("Failed to serialize quote: {}", e),
                    );
                    return std::ptr::null_mut();
                }
            };
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_createFriendRequestHiddenService(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Creating friend request hidden service on port {} -> {}, dir: {}",
                service_port,
                local_port,
                dir_str
            );
            ...
            let fr_private_key = match crate::ffi::keystore::get_friend_request_private_key(
                &mut env,
                &key_manager,
            ) {
                Ok(k) => k,
                Err(e) => {
                    let _ = env.throw_new(
                        "java/lang/RuntimeException",
                        format!("Failed to get friend request key: {}", e),
                    );
                    return std::ptr::null_mut();
                }
            };
            ...
            let result = GLOBAL_RUNTIME.block_on(async {
                let mut manager = tor_manager.lock().unwrap();
                manager
                    .create_hidden_service(service_port as u16, local_port as u16, &fr_private_key)
                    .await
            });
            ...
            match result {
                Ok(onion_address) => {
                    log::info!(
                        "Friend request hidden service created successfully: {}",
                        onion_address
                    );
                ...
                Err(e) => {
                    let error_msg =
                        format!("Failed to create friend request hidden service: {}", e);
                    log::error!("{}", error_msg);
                    let _ = env.throw_new("java/lang/RuntimeException", error_msg);
                    std::ptr::null_mut()
                }
            ...
            std::thread::spawn(move || {
                ...
                rt.block_on(async {
                    ...
                    if let Err(e) = endpoint.start(port as u16).await {
                        ...
                        log::info!(
                            "Contact exchange endpoint started successfully on port {}",
                            port
                        );
                    }
                });
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_serveContactCard(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Storing encrypted contact card (CID: {}, length: {})",
                cid_str,
                card_length
            );
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_serveContactList(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Storing encrypted contact list (CID: {}, length: {})",
                cid_str,
                list_length
            );
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_setRecoveryMode(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "setRecoveryMode: enabled={}, has_cid={}, has_dir={}",
                enabled,
                cid_str.is_some(),
                dir_str.is_some()
            );
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_httpPostBinaryViaTor(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "HTTP POST binary via Tor: {} ({} bytes)",
                url_str,
                data_bytes.len()
            );
                ...
                Ok(response) => {
                    if let Some(body) = crate::network::socks5_client::extract_http_body(&response)
                    {
                        match string_to_jstring(&mut env, &body) {
                            Ok(s) => {
                                log::info!(
                                    "HTTP POST binary successful ({} bytes response)",
                                    body.len()
                                );
                                s.into_raw()
                            }
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_createVoiceSession(
    ...
    catch_panic!(
        env,
        {
            ...
            log::info!(
                "Creating voice session with {} circuits for call: {} to {}",
                num_circuits_usize,
                call_id_str,
                peer_onion_str
            );
            ...
            let result = GLOBAL_RUNTIME.block_on(async {
                let mut listener = voice_listener.lock().await;
                listener
                    .create_session(call_id_str.clone(), &peer_onion_str, num_circuits_usize)
                    .await
            });
            ...
            match result {
                Ok(_) => {
                    log::info!(
                        "Voice session with {} circuits created for call: {}",
                        num_circuits_usize,
                        call_id_str
                    );
                    1
    ...
    catch_panic!(
        env,
        {
            ...
            match listener.send_audio(&call_id_str, circuit_index as usize, packet) {
                ...
                Err(e) => {
                    log::warn!(
                        "Failed to send audio packet for call {} on circuit {}: {}",
                        call_id_str,
                        circuit_index,
                        e
                    );
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_rebuildVoiceCircuit(
    ...
    catch_panic!(
        env,
        {
            ...
            log::warn!(
                "Rebuilding voice circuit {} for call {} (rebuild_epoch={})",
                circuit_index,
                call_id_str,
                rebuild_epoch
            );
            ...
            let result = GLOBAL_RUNTIME.block_on(async {
                ...
                listener
                    .rebuild_circuit(&call_id_str, circuit_index as usize, rebuild_epoch as u32)
                    .await
            });
            ...
            match result {
                Ok(_) => {
                    log::info!(
                        "Circuit {} rebuild SUCCESS for call {}",
                        circuit_index,
                        call_id_str
                    );
                    1 // success
                ...
                Err(e) => {
                    log::error!(
                        "Circuit {} rebuild FAILED for call {}: {}",
                        circuit_index,
                        call_id_str,
                        e
                    );
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_verifyRangeProof(
    ...
    catch_panic!(
        env,
        {
            ...
            let commitment_vec = env
                .convert_byte_array(&commitment_bytes)
                .expect("Failed to read commitment bytes");

            if commitment_vec.len() != 32 {
                log::error!(
                    "verifyRangeProof: commitment must be 32 bytes, got {}",
                    commitment_vec.len()
                );
                return JNI_FALSE;
            }
...
pub extern "C" fn Java_com_shieldmessenger_crypto_RustBridge_xchacha20Encrypt(
    ...
    catch_panic!(
        env,
        {
            ...
            let cipher = XChaCha20Poly1305::new_from_slice(&key_vec).expect("Invalid key length");
            let nonce_obj = XNonce::from_slice(&nonce_vec);

            match cipher.encrypt(nonce_obj, plaintext_vec.as_ref()) {
                Ok(ciphertext) => match vec_to_jbytearray(&mut env, &ciphertext) {
                    Ok(arr) => arr.into_raw(),
                    Err(e) => {
                        let _ = env.throw_new("java/lang/RuntimeException", e);
                        std::ptr::null_mut()
    ...
    catch_panic!(
        env,
        {
            ...
            match cipher.decrypt(nonce_obj, ciphertext_vec.as_ref()) {
                Ok(plaintext) => match vec_to_jbytearray(&mut env, &plaintext) {
                    Ok(arr) => arr.into_raw(),
                    Err(e) => {
                        let _ = env.throw_new("java/lang/RuntimeException", e);
                        std::ptr::null_mut()
        ...
        env,
        {
            match blocking_recv_pair(&GLOBAL_PONG_RECEIVER, 5) {
                Some((conn_id, pong_bytes)) => {
                    ...
                    log::info!(
                        "POLL_BLOCKING: got pong len={} conn={}",
                        pong_bytes.len(),
                        conn_id
                    );
                    match vec_to_jbytearray(&mut env, &pong_bytes) {
        ...
        env,
        {
            match blocking_recv_vec(&GLOBAL_FRIEND_REQUEST_RECEIVER, 5) {
                Some(friend_request_bytes) => {
                    log::info!(
                        "Polled friend request (blocking): {} bytes",
                        friend_request_bytes.len()
                    );
                    match vec_to_jbytearray(&mut env, &friend_request_bytes) {
                        Ok(array) => array.into_raw(),
    ...
    catch_panic!(
        env,
        {
            ...
            let json = match result {
                ...
                crate::crypto::pqc::IdentityKeyChangeResult::Changed {
