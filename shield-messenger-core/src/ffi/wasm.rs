...
///   wasm-bindgen --out-dir pkg --target web target/wasm32-unknown-unknown/release/shieldmessenger.wasm
...
pub fn derive_root_key(shared_secret_b64: &str) -> Result<String, JsValue> {
    ...
    let root_key = encryption::derive_root_key(&shared_secret, b"ShieldMessenger-RootKey-v1")
        .map_err(|e| JsValue::from_str(&e.to_string()))?;
