...
fuzz_target!(|data: &[u8]| {
    ...
    let _ = shieldmessenger::network::packet::Packet::deserialize(data, &hmac_key);

    // If we have a valid-looking payload, try round-trip
    let payload_data = &data[32..];
    if payload_data.len() <= shieldmessenger::network::packet::MAX_PAYLOAD {
        if let Ok(pkt) = shieldmessenger::network::packet::Packet::new(
            shieldmessenger::network::packet::PacketType::Message,
            payload_data.to_vec(),
        ) {
            if let Ok(serialized) = pkt.serialize(&hmac_key) {
                let deserialized = shieldmessenger::network::packet::Packet::deserialize(
                    &serialized, &hmac_key
