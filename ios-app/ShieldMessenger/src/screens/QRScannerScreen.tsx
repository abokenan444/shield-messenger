  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.cameraPlaceholder}>
        ...
        <Text style={styles.scanText}>{scanning ? t('point_camera') : t('processing')}</Text>
      </View>

      <View style={styles.instructions}>
        <Text style={styles.instructionTitle}>{t('verify_contact_identity')}</Text>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoText}>{t('verified_trust_level_2')}</Text>
        </View>
        <View style={styles.infoRow}>
          <Text style={styles.infoIcon}>📱</Text>
          <Text style={styles.infoText}>{t('both_devices_present')}</Text>
        </View>
      </View>

      <TouchableOpacity style={styles.showMyQR} onPress={() => navigation.navigate('QRCode')}>
        <Text style={styles.showMyQRText}>{t('show_my_qr')}</Text>
      </TouchableOpacity>
    </View>
  );
};
