...
fuzz_target!(|input: RatchetInput| {
    ...
    let alice_x25519 = securelegion::crypto::key_exchange::generate_x25519_keypair_from_seed(&alice_dh_secret);
    let bob_x25519 = securelegion::crypto::key_exchange::generate_x25519_keypair_from_seed(&bob_dh_secret);

    // Generate PQ keypairs
    let alice_kem = match securelegion::crypto::pqc::generate_hybrid_keypair() {
        Ok(kp) => kp,
        Err(_) => return,
    };
    let bob_kem = match securelegion::crypto::pqc::generate_hybrid_keypair() {
        Ok(kp) => kp,
        Err(_) => return,
    };

    // Compute shared secret for initialization
    let shared_secret = match securelegion::crypto::key_exchange::x25519_key_exchange(
    ...
    // Initialize ratchets
    let mut alice_ratchet = securelegion::crypto::ratchet::PQDoubleRatchet::init_alice(
        shared_secret.clone(),
        alice_x25519.1,
        bob_x25519.1,
        Some(alice_kem),
    ...
    let mut bob_ratchet = securelegion::crypto::ratchet::PQDoubleRatchet::init_bob(
        shared_secret,
        bob_x25519.1,
        bob_x25519.0,
        Some(bob_kem),
    ...
    let mut encrypted: Vec<(securelegion::crypto::ratchet::RatchetHeader, Vec<u8>)> = Vec::new();
    for msg in &input.messages {
    ...
    let _restored = securelegion::crypto::ratchet::PQDoubleRatchet::import_state(state);
});
